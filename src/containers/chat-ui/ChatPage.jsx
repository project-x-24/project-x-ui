...
import { LeftArrowIcon, SmileIcon, AttachmentIcon, CameraIcon, SendIcon, SettingsIcon } from "../../assets";
import EmojiPicker from "emoji-picker-react";
import { dummyData } from "../../constants/dummyData";
import { LayoutContextProvider, LiveKitRoom, RoomAudioRenderer, useChat, useLocalParticipant, useTrackTranscription, useVoiceAssistant } from "@livekit/components-react";
	...
	return (
		<LayoutContextProvider>
			<div className="flex flex-col h-screen w-screen">
				...
				<div className="flex items-center justify-between py-3 px-[24px] bg-white shadow-md border-b border-[#F4F4F4]">
					<div className="flex">
						...
						<div className="flex w-full last:relative items-center">
							<img alt="" src={activePersona?.imageSrc} className="rounded-full w-10 h-10 ml-3 object-cover" />
							<div className="ml-3 flex-grow">
								<p className="font-bold text-[rgba(16,25,40,1)]">{activePersona?.persona}</p>
							</div>
						</div>
					</div>
				...
				<LiveKitRoom token={activePersona?.token} serverUrl={activePersona?.serverUrl} connectOptions={{ autoSubscribe: true }}>
					<ActiveRoom isVoice={isVoice} setIsVoice={setIsVoice} ActivePersona={activePersona} isVoiceAgent={isVoiceAgent} />
				</LiveKitRoom>
			</div>
		</LayoutContextProvider>
	);
		...
		<>
			...
			{isVoiceAgent && (
				<VoiceOverlay
					onClick={() => {
						setIsVoice(true);
						localParticipant?.localParticipant?.setMicrophoneEnabled(!isMicrophoneEnabled);
					}}
			...
			{isVoice && (
				<main
					...
					<div className="h-full flex flex-col items-center relative">
						<div className="text-black text-[32px] font-normal mt-[140px]">{ActivePersona?.persona}</div>
						<div className="text-[#656464A8] opacity-[0.6] text-[21px] font-normal mt-[1px]">{ActivePersona?.token ? "Connected" : "Connecting..."}</div>
						<img src={ActivePersona?.imageSrc} className="rounded-full w-[172px] h-[172px] object-cover mt-[26px]" />
			...
			<div className="relative flex flex-col h-[calc(100vh-74px)]">
				<div className="w-full text-center capitalize p-1 bg-[#f4f4f4]">{voiceAssistant.state}</div>
				...
				<div className="absolute bottom-0 flex flex-col w-screen p-3 items-center bg-white" style={{ boxShadow: "0px 0px 4px 0px rgba(0, 0, 0, 0.12)" }}>
