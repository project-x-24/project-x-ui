...
function HomePage({ name }) {
	const categories = [
		{ name: "Chat", bgStyle: "bg-gradient-to-r from-[#7dc8e7] to-[#7dc8e7b0]", icon: <ChatBg /> },
		{ name: "Memory Games", bgStyle: "bg-gradient-to-r from-[#7D88E7] to-[#8C96F0A3]", icon: <MemoryBg /> },
		{ name: "Voice Assist", bgStyle: "bg-gradient-to-r from-[#E77D7D] to-[#E77D7DB5]", icon: <VoiceAssistBg /> },
		{ name: "Face Assist", bgStyle: "bg-gradient-to-r from-[#498A5B] to-[#7ACB90]", icon: <FaceAssistBg /> },
	];
	...
	return (
		<div className="min-h-screen p-6 w-screen bg-white">
			...
			{/* Categories */}
			<span className="text-[16px] font-[500] text-[#1C1C1C]">Categories</span>
			<div className="grid grid-cols-2 gap-4 mt-3 mb-7">
				{categories.map((category, index) => (
					<div key={index} className={`p-16 rounded-[14px] h-[163px] shadow-md flex flex-col justify-center items-center relative font-bold text-center text-white ${category.bgStyle}`}>
						{/* Custom Icon for each category */}
						<div className="mb-4 absolute top-0 left-0 overflow-hidden">{category.icon}</div>
						<div className="absolute bottom-[16px] flex justify-between items-center w-full px-6">
							<span className="text-[18px] text-left font-[500]">{category.name}</span>
							<RightIcon />
						</div>
					</div>
				))}
			...
			<div className="space-y-4">
				{notes.map((note, index) => (
					<div key={index} className="bg-white p-4 rounded-lg shadow-md" style={{ boxShadow: "0px 4px 4px 0px #00000014" }}>
						...
						<div className="flex justify-between items-center">
							<div className="flex items-center">
								<ClockIcon />
								<span className="text-[14px] h-5 text-[#98A2B3] align-middle font-[400] ml-1">{note.time}</span>
							</div>
